...
export default function ReportFiltering({
  ...
  const getDefaultDates = () => {
    ...
    if (
      earliestDate.getTime() !== initialEarliest.getTime() &&
      latestDate.getTime() !== initialLatest.getTime()
    ) {
  ...
  const generateReport = async () => {
    ...
    if (startDateError || endDateError) {
      setDateErrors({ startDate: startDateError, endDate: endDateError });
      setErrorMessage(
        "Por favor, corrija os erros de data antes de gerar o relatório",
      );
      setErrorAlert(true);
      setIsGeneratingReport(false);
      return;
    ...
    const filteredTransactions = transactions.filter((transaction) => {
      ...
      try {
        if (transaction.start_date) {
          ...
          if (isNaN(transactionStartDate.getTime())) {
            console.warn(
              `Invalid start_date for transaction ${transaction.id}:`,
        ...
        if (transaction.end_date) {
          ...
          if (isNaN(transactionEndDate.getTime())) {
            console.warn(
              `Invalid end_date for transaction ${transaction.id}:`,
        ...
        console.error(
          `Error parsing dates for transaction ${transaction.id}:`,
          error,
        );
      }
      ...
      const matchesTransactionType = selectedTransactions.includes(
    ...
    if (filteredTransactions.length === 0) {
      setErrorMessage(
        "Não há transacções para o período e opções selecionadas",
      );
      setErrorAlert(true);
      setIsGeneratingReport(false);
      return;
  ...
  return (
    <View className="flex-1 py-2 gap-y-6">
        ...
        <View className="flex-row flex-wrap">
          {transactionTypes.map((type) => (
            ...
            >
              <Checkbox
                status={
                  selectedTransactions.includes(type.id)
                    ? "checked"
                    : "unchecked"
                }
                onPress={() => toggleTransaction(type.id)}
              />
              <Text className="ml-2 text-gray-600 dark:text-gray-400 text-[13px]">
                {type.label}
              </Text>
            </TouchableOpacity>
          ))}
