...
export const useUserFromPowerSync = () => {
  const [userDetails, setUserDetails] = useState<UserDetailsRecord | null>(
    null,
  );
...
export const useUserSession = () => {
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (
        event === AUTH_EVENTS.SIGNED_OUT ||
        (event === AUTH_EVENTS.TOKEN_REFRESHED && !session)
      ) {
        setSession(null);
        setIsLoading(false);
        setIsError(false);
...
export const useQueryOneAndWatchChanges = <T>(
  query: string,
  params: string[],
) => {
  const [data, setData] = useState<T | null>(null);
  ...
  useEffect(() => {
    ...
    powersync.watchWithCallback(
      ...
      params,
      {
        ...
        onError: (error) => {
          console.log(
            "PowerSync query error (useQueryOneAndWatchChanges):",
            error,
          );
  ...
  useEffect(() => {
    ...
    powersync.watchWithCallback(
      ...
      [],
      {
        ...
        onError: (error) => {
          console.log(
            "PowerSync query error (useQueryManyAndWatchChanges):",
            error,
          );
          setError(`PowerSync query error: ${error}`);
          setIsError(true);
...
export const useSearchOptions = (userDistrictId?: string) => {
  ...
  const loadSearchKeys = useCallback(async () => {
    try {
      ...
      const searchOptions = adminPosts
        ? adminPosts.map((post) => ({ label: post.name, value: post.id }))
        : [];
...
export const useGroupedOrganizations = <
  T extends { organization_type: string },
>(
  organizations: T[],
) => {
  return useMemo(() => {
