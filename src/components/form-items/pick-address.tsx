...
export function FromAdminPosts({ control, customErrors, districtId, description }: Props) {
	...
	const safePartialAddress = partialAddress ?? { adminPostId: null, villageId: null }
	const safeDistrictId = districtId && typeof districtId === 'string' ? districtId.trim() : ''
					...
					render={({ field: { onChange, value, onBlur }, fieldState: { error } }) => (
						<View>
							<PickLocationName
								...
								referenceId={safeDistrictId !== '' ? safeDistrictId : undefined}
							/>
					...
					render={({ field: { onChange, value, onBlur }, fieldState: { error } }) => (
						<View>
							<PickLocationName
								...
								referenceId={
									safePartialAddress.adminPostId && typeof safePartialAddress.adminPostId === 'string' && safePartialAddress.adminPostId.trim() !== ''
...
export function FromProvinces({ control, errors, customErrors, clearFieldError, description }: Props) {
	...
	// Reset dependent fields when parent changes - minimal deps to avoid cascading updates
	useEffect(() => {
		if (safeFullAddress.provinceId) {
			resetFullDistrictId()
			resetFullAdminPostId()
			resetFullVillageId()
		}
					...
					render={({ field: { onChange, value, onBlur }, fieldState: { error } }) => (
						<View>
							<PickLocationName
								...
								referenceId={
									safeFullAddress.provinceId && typeof safeFullAddress.provinceId === 'string' && safeFullAddress.provinceId.trim() !== ''
										? safeFullAddress.provinceId
										: undefined
								}
							/>
					...
					render={({ field: { onChange, value, onBlur }, fieldState: { error } }) => (
						<View>
							<PickLocationName
								...
								locationType={LocationType.ADMIN_POST}
								referenceId={
									safeFullAddress.districtId && typeof safeFullAddress.districtId === 'string' && safeFullAddress.districtId.trim() !== ''
										? safeFullAddress.districtId
										: undefined
								}
							/>
					...
					render={({ field: { onChange, value, onBlur }, fieldState: { error } }) => (
						<View>
							<PickLocationName
								...
								locationType={LocationType.VILLAGE}
								referenceId={
									safeFullAddress.adminPostId && typeof safeFullAddress.adminPostId === 'string' && safeFullAddress.adminPostId.trim() !== ''
