...
export const updateUserDetails = async (
  ...
  const { data, error: userDetailsError } = await (
    supabase.from(TABLES.USER_DETAILS) as any
  )
    .update(updates)
    .eq("email", email)
    .select()
...
export const fetchUserDetailsFromSupabase =
  async (): Promise<UserDetailsRecord | null> => {
    try {
      ...
      if (data) {
        console.log(
          "✅ Successfully fetched user details from Supabase:",
          data,
        );
...
export const userSignUp = async (
  ...
  if (
    existingUser &&
    existingUser.status ===
      AUTH_CODES.USER_DETAILS_STATUS.EMAIL_PENDING_VERIFICATION
  ) {
    const {
  ...
  if (
    existingUser &&
    existingUser.status === AUTH_CODES.USER_DETAILS_STATUS.AUTHORIZED
  ) {
    return {
      success: false,
      message:
        "Email já autorizado! Por favor, faça login com o email e senha que você usou para criar a conta!",
      session: null,
  ...
  if (
    existingUser &&
    existingUser.status === AUTH_CODES.USER_DETAILS_STATUS.BLOCKED
  ) {
    return {
      success: false,
  ...
  if (
    existingUser &&
    existingUser.status === AUTH_CODES.USER_DETAILS_STATUS.BANNED
  ) {
    return {
      success: false,
  ...
  if (
    existingUser &&
    existingUser.status === AUTH_CODES.USER_DETAILS_STATUS.UNAUTHORIZED
  ) {
    return {
      success: false,
      message:
        "Email ainda não autorizado! Por favor, contacte a equipe de suporte.",
      session: null,
  ...
  if (error) {
    ...
    if (error.code === AUTH_CODES.INVALID_CREDENTIALS) {
      return {
        success: false,
        message:
          "Email ou senha inválidos! Por favor, confirme as credenciais fornecidas e tente novamente.",
        session: null,
        code: error.code,
      };
    }
...
export const getUserDetailsByEmail = async (
  email: string,
): Promise<UserDetailsRecord | null> => {
  try {
    const { data, error } = await supabase
      .from(TABLES.USER_DETAILS)
      .select("*")
  ...
  try {
    ...
    if (error) {
      ...
      return {
        success: false,
        message:
          "Erro ao reenviar email de verificação. Por favor, tente novamente mais tarde!",
...
export const resetPassword = async (password: string) => {
  try {
    ...
    if (error) {
      ...
      return {
        success: false,
        message: "Erro ao redefinir senha. Escolha uma senha diferente.",
        code: AUTH_CODES.PASSWORD_RESET_ERROR,
      };
    }
    return {
