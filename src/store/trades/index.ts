...
export interface TransferredByOrgInfo extends TransferredByOrgInfoState {
  ...
  setTransferredByOrgInfo: (
    transferredByOrgInfo: TransferredByOrgInfoState,
  ) => void;
...
export interface TransactedItemState {
  item: string;
  setItem: (item: string) => void;
  resetItem: () => void;
  getItem: () => TransactedItemState;
  assertItem: () => { status: boolean; message: string };
}
...
export const useInfoProviderStore = create<InfoProviderState>((set, get) => ({
  ...
  setHasSelectedInfoProvider: (hasSelectedInfoProvider: boolean) =>
    set({ hasSelectedInfoProvider }),
  setInfoProvider: (infoProvider: InfoProviderData) => set({ infoProvider }),
  resetInfoProvider: () =>
    set({
      hasSelectedInfoProvider: false,
      infoProvider: { info_provider_id: "", info_provider_name: "" },
    }),
  getInfoProvider: () => get(),
  assertInfoProvider: () => {
    ...
    if (!hasSelectedInfoProvider) {
      return {
        status: false,
        message: "Selecione um fornecedor de informações.",
      };
    }
  ...
  assertBoughtInfo: () => {
    ...
    if (hasBought) {
      if (!boughtPrice && !quantityBought) {
        return {
          status: false,
          message: "As informações de compras são obrigatórias.",
      ...
      if (boughtPrice && !quantityBought) {
        return {
          status: false,
          message: "Indica a quantidade comprada.",
          quantity: quantityBought,
        };
      }
      if (!boughtPrice && quantityBought) {
        return {
          status: false,
          message: "Indica o preço de compra.",
          quantity: quantityBought,
        };
      }
      if (boughtPrice <= 5) {
        return {
          status: false,
          message: "O preço de compra deve ser maior que 5 MZN.",
          quantity: quantityBought,
        };
      }
      if (quantityBought <= 5) {
        return {
          status: false,
          message: "A quantidade comprada deve ser maior que 5 Kg.",
          quantity: quantityBought,
        };
      }
    }
  ...
  assertResoldInfo: () => {
    ...
    if (hasResold) {
      if (!resoldPrice && !quantityResold) {
        return {
          status: false,
          message: "As informações de vendas são obrigatórias.",
      ...
      if (resoldPrice && !quantityResold) {
        return {
          status: false,
          message: "Indica a quantidade vendida.",
          quantity: quantityResold,
        };
      }
      if (!resoldPrice && quantityResold) {
        return {
          status: false,
          message: "Indica o preço de venda.",
          quantity: quantityResold,
        };
      }
      if (resoldPrice <= 5) {
        return {
          status: false,
          message: "O preço de venda deve ser maior que 5 MZN.",
          quantity: quantityResold,
        };
      }
      if (quantityResold <= 5) {
        return {
          status: false,
          message: "A quantidade vendida deve ser maior que 5 Kg.",
          quantity: quantityResold,
        };
      }
    }
...
export const useAggregatedInfoStore = create<AggregatedInfo>((set, get) => ({
  ...
  setAggregatedInfo: (aggregatedInfo: AggregatedInfoState) =>
    set(aggregatedInfo),
  resetAggregatedInfo: () =>
    set({ hasAggregated: false, activeParticipantParticipations: [] }),
...
export const useTransferredByOrgInfoStore = create<TransferredByOrgInfo>(
  (set, get) => ({
    hasTransferredByOrg: false,
    transfersOrganizations: [],
    setHasTransferredByOrg: (hasTransferredByOrg: boolean) =>
      set({ hasTransferredByOrg }),
    setTransferredByOrgInfo: (
      transferredByOrgInfo: TransferredByOrgInfoState,
    ) => set(transferredByOrgInfo),
    resetTransferredByOrgInfo: () =>
      set({ hasTransferredByOrg: false, transfersOrganizations: [] }),
    ...
    removeOrganization: ({ group_id }: { group_id: string }) =>
      set((state) => ({
        transfersOrganizations: state.transfersOrganizations.filter(
          (o) => o.group_id !== group_id,
        ),
      })),
    assertTransferredByOrgInfo: () => {
      ...
      if (hasTransferredByOrg) {
        if (transfersOrganizations.length === 0) {
          return {
            ...
            quantity: transfersOrganizations.reduce(
...
export const useTransferredInfoStore = create<TransferredInfo>((set, get) => ({
  ...
  setTransferredInfo: (transferredInfo: TransferredInfoState) =>
    set(transferredInfo),
  resetTransferredInfo: () =>
    set({ hasTransferred: false, transfersWarehouses: [] }),
  ...
  removeWarehouse: ({ warehouse_id }) =>
    set((state) => ({
      transfersWarehouses: state.transfersWarehouses.filter(
...
export const useExportationInfoStore = create<ExportationInfo>((set, get) => ({
  ...
  exportations: [],
  setHasSentToExportation: (hasSentToExportation: boolean) =>
    set({ hasSentToExportation }),
  setExportationInfo: (exportationInfo: ExportationInfoState) =>
    set(exportationInfo),
...
export const useProcessingInfoStore = create<ProcessingInfo>((set, get) => ({
  ...
  processingWarehouses: [],
  setHasSentToProcessing: (hasSentToProcessing: boolean) =>
    set({ hasSentToProcessing }),
  ...
  removeWarehouse: ({ warehouse_id }: { warehouse_id: string }) =>
    set((state) => ({
      processingWarehouses: state.processingWarehouses.filter(
        (w) => w.warehouse_id !== warehouse_id,
      ),
    })),
  assertProcessingInfo: () => {
    ...
    if (hasSentToProcessing) {
      if (processingWarehouses.length === 0) {
        return {
          status: false,
          message: "Indica pelo menos uma transferência.",
          quantity: processingWarehouses.reduce(
            (acc, w) => acc + w.quantity,
            0,
          ),
        };
      }
  ...
  assertDateRange: () => {
    ...
    if (diffDays < 1) {
      return {
        status: false,
        message:
          "O intervalo de tempo entre a data de início e fim deve ser de pelo menos 1 dia.",
      };
    }
    return { status: true, message: "" };
  },
...
export const useTransactedItemStore = create<TransactedItemState>(
  (set, get) => ({
    item: "",
    ...
    assertItem: () => {
      ...
      if (!item) {
        return {
          status: false,
          message: "Selecione um item para a transação.",
        };
      }
      return { status: true, message: "" };
    },
