...
function useSegmentCompletion() {
  const { formData } = useAssociationStore();
...
export default function AssociationSegmentedDataForm() {
  const router = useRouter();
  const { userDetails } = useUserDetails();
  const { resetFormData, formData } = useAssociationStore();
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (allComplete && !hasError) {
      ...
      const { success, message } = await insertGroup({
        ...
        groupType: "ASSOCIATION" as OrganizationTypes,
      ...
      if (success) {
        ...
        setSuccess(true);
        setTimeout(() => {
          router.replace("/(tabs)/actors/associations" as Href);
        }, 400);
      } else {
  ...
  return (
    <View className="flex-1">
            ...
            <View key={segment.id} className="mb-3">
              ...
              {segment.id === "basicInfo" ? (
                <GroupBasicInfoSegment
                  ...
                  variant="inline"
                  groupType={OrganizationTypes.ASSOCIATION}
                />
              ) : null}
      ...
      <SuccessAlert
        ...
        route={"/(tabs)/actors/associations" as Href}
      />
    </View>
  );
}
