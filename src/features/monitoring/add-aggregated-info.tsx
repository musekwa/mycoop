...
import ParticipantSelectModal from "@/components/modals/custom-multi-selector";
...
import { useAggregatedInfoStore, useTransactedItemStore } from "@/store/trades";
...
export default function AddAggregatedInfo({
  ...
  const flattenedMembers = useMemo(() => {
    ...
    const mapped = allMembers
      ...
      .map((m) => {
        return {
          label: `${m.other_names} ${m.surname}  `,
          value: m.member_id as string,
          photo: m.photo,
          uaid: m.uaid,
        };
      });
  ...
  const activeParticipantParticipations = watch(
    "activeParticipantParticipations",
  );
  ...
  const handleRemoveMember = (participantId: string) => {
    const updatedParticipations = activeParticipantParticipations.filter(
  ...
  const validateAggregatedInfo = () => {
    ...
    const activeParticipantParticipations = getValues(
      "activeParticipantParticipations",
    );
      ...
      <View className="flex-row items-center justify-between mb-4">
        <View className="flex-1">
          <Text className="text-sm text-gray-600 dark:text-gray-400">
            Houve agregação de {itemType.toLowerCase()} pelos membros?
          </Text>
        </View>
        <Controller
          control={control}
      ...
      {hasAggregatedValue && (
        <View className="flex-1">
          ...
          {activeParticipantParticipations.map((activeParticipant) => (
            <View
              key={activeParticipant.participant_id}
            ...
            >
              <View className="flex-row justify-between items-center mb-2">
                ...
                <TouchableOpacity
                  onPress={() =>
                    handleRemoveMember(activeParticipant.participant_id)
                  }
                >
                  ...
                  <Controller
                    ...
                    render={({
                      ...
                      <View className="flex-1">
                          ...
                          <CustomTextInput
                            ...
                            onChangeText={(text) => {
                              ...
                              setAggregatedInfo({
                                hasAggregated: hasAggregatedValue,
                                activeParticipantParticipations:
                                  activeParticipantParticipations.map((m) =>
                                    m.participant_id ===
                        ...
                        <Text className="text-gray-400 italic text-[12px]">
                          Qt. agregada
                        </Text>
                      </View>
                    )}
          ...
          <View className="relative">
            <ParticipantSelectModal
              showModal={showMemberSelect}
              setShowModal={setShowMemberSelect}
              onClose={() => setShowMemberSelect(false)}
              options={
                flattenedMembers && flattenedMembers.length > 0
                  ? flattenedMembers.filter(
                      (m) =>
                        !activeParticipantParticipations.some(
                          (p) => p.participant_id === m.value,
                        ),
                    )
                  : []
              }
              ...
              label="Seleccione os membros que participaram da agregação"
