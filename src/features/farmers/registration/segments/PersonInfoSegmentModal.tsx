...
const PersonInfoSchema = z.object({
	surname: z.string().trim().min(2, 'Indica um apelido.').regex(/^\S*$/, 'Apenas um apelido.'),
	...
	familySize: z
		...
		.max(20, 'Máx. 20')
		.optional()
		.refine((val): val is number => val != null, { message: 'Indica o agregado familiar' }),
...
export default function PersonInfoSegmentModal({ visible, onClose }: Props) {
	...
	useEffect(() => {
		if (visible) {
			reset({
				...
				familySize: person?.familySize != null ? Number(person.familySize) : undefined,
	...
	return (
		<SegmentModalLayout visible={visible} title="Informação Pessoal" onClose={onClose} onSave={onSave}>
			...
			<Controller
				...
				render={({ field: { onChange, value } }) => (
					<View className="mt-4">
						...
						/>
						{errors.surname && (
							<Text className="text-xs text-red-500">{errors.surname.message}</Text>
						)}
					</View>
				)}
			/>
			...
			<Controller
				...
				render={({ field: { onChange, value } }) => (
					<View className="mt-4">
						...
						/>
						{errors.otherNames && (
							<Text className="text-xs text-red-500">{errors.otherNames.message}</Text>
						)}
					</View>
				)}
			/>
			...
			<Controller
				...
				render={({ field: { onChange, value } }) => (
					<View className="mt-4">
						<Text className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Gênero</Text>
						<View className="flex-row justify-around">
							...
							>
								...
								<Text className="text-gray-700 dark:text-gray-300">Homem</Text>
							</TouchableOpacity>
							<TouchableOpacity
							...
							>
								...
								<Text className="text-gray-700 dark:text-gray-300">Mulher</Text>
							</TouchableOpacity>
						</View>
						{errors.gender && (
							<Text className="text-xs text-red-500">{errors.gender.message}</Text>
						)}
					</View>
				)}
			/>
			...
			<Controller
				...
				render={({ field: { onChange, value } }) => (
					<View className="mt-4">
						...
						/>
						{errors.familySize && (
							<Text className="text-xs text-red-500">{errors.familySize.message}</Text>
						)}
					</View>
				)}
			/>
