...
export default function FarmersScreen() {
  ...
  const filteredFarmers = useMemo(() => {
    ...
    if (newSearchKey) {
      ...
      return farmersWithAdminPostAndContact
        .filter((farmer) =>
          farmer?.admin_post_id
            ?.toLowerCase()
            .includes(newSearchKey.toLowerCase()),
        )
        .reverse();
    }
    return farmersWithAdminPostAndContact
      ...
      .reverse();
  }, [search, farmersWithAdminPostAndContact, newSearchKey]);
  ...
  return (
    <CustomSafeAreaView edges={["bottom", "top"]} style={{ paddingTop: 0 }}>
      <View className="px-2">
        {isLoading ? (
          <FlatList
