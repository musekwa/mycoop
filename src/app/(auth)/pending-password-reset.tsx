  ...
  const handleSubmit = async () => {
    try {
      ...
      if (fullCode.length === 6) {
        const { code, message, success } = await comparePasswordResetCode(
          currentEmail,
          fullCode,
        );
  ...
  const resendCode = async () => {
    ...
    try {
      const {
        ...
        code: resendCode,
      } = await forgotPassword(emailToCheck);
    ...
    <CustomSafeAreaView>
      ...
      >
        ...
        >
          <HeroCard
            title="MyCoop"
            description={`Verifique seu email. Um código de redefinição de senha de 6 dígitos foi enviado para ${currentEmail}`}
          />
