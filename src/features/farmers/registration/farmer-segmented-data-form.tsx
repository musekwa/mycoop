...
function useSegmentCompletion() {
  ...
  const isBirthPlaceComplete = (() => {
    ...
    if (birthPlace.nationality === "NATIONAL") {
      ...
      return Boolean(
        fa.provinceId && fa.districtId && fa.adminPostId && fa.villageId,
      );
    }
    if (birthPlace.nationality === "FOREIGN") {
      return Boolean(
        birthPlace.countryId && birthPlace.countryId.trim() !== "",
      );
    }
    return false;
  })();
  ...
  const isDocumentationComplete = Boolean(
    ...
    (documentation.docType === "Não tem" ||
      (documentation.docNumber && documentation.docNumber.trim() !== "")),
  );
...
export default function FarmerSegmentedDataForm() {
  ...
  const { resetAll, syncToFarmerAndAddressStores } =
    useFarmerRegistrationStore();
  ...
  return (
    <View className="flex-1">
      ...
      <SuccessAlert
        ...
        route={"/(tabs)/actors/farmers" as Href}
      />
    </View>
  );
}
