...
export default function AddressSegmentModal({ visible, onClose }: Props) {
	...
	const { partialAddress, setPartialAdminPostId, setPartialVillageId } = useAddressStore()
	...
	useEffect(() => {
		if (visible) {
			...
			const adminPostId = savedAddress?.adminPostId ?? partialAddress?.adminPostId ?? ''
			const villageId = savedAddress?.villageId ?? partialAddress?.villageId ?? ''
			setPartialAdminPostId(adminPostId || undefined)
			setPartialVillageId(villageId || undefined)
			reset({ adminPostId, villageId })
		}
	}, [visible, savedAddress, partialAddress?.adminPostId, partialAddress?.villageId, reset, setPartialAdminPostId, setPartialVillageId])
	...
	return (
		<SegmentModalLayout visible={visible} title="Endereço" onClose={onClose} onSave={onSave}>
