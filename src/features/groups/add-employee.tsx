...
const WorkerSchema = z.object({
  ...
  position: z
    .string()
    .trim()
    .min(2, "Indica a função que desempenha.")
    .optional(),
  phone: z
    .string()
...
export default function AddEmployee() {
  ...
  const { otherNames, surname, photo, contactId } = useTraderById(
  ...
  const [isFacilityOwner, setIsFacilityOwner] = useState<"YES" | "NO" | null>(
    null,
  );
  ...
  useEffect(() => {
    if (isFacilityOwner === "YES") {
      ...
      setValue(
        "phone",
        String(primaryPhone) || String(secondaryPhone) || undefined,
      );
    ...
    async (data: WorkerFormData) => {
      ...
      if (result.success) {
        if (!result.data.selectedWarehouse) {
          setHasError(true);
          setErrorMessage(
            "Seleccione o armazém onde o trabalhador está afetado.",
          );
          return;
        }
        ...
        if (!getCurrentResource().id) {
          setHasError(true);
          setErrorMessage(
            "Não foi possível encontrar o comerciante proprietário do armazém.",
          );
          return;
        }
        ...
        try {
          if (
            ...
            const address_detail_row = buildAddressDetail({
              ...
              sync_id:
                warehouseAddress?.sync_id ||
                result.data.selectedWarehouse?.addressId ||
                "",
            });
      ...
      >
        {!isCompany && (
          <View>
            <Text className="text-black dark:text-white mb-2 text-[12px] italic">
              Este trabalhador é o mesmo que o comerciante proprietário do
              armazém?
            </Text>
                ...
                <RadioButton
                  ...
                  onPress={() => {
                    ...
                    reset({
                      ...
                      selectedWarehouse: undefined,
                      isFacilityOwner: "NO",
                    }); // Reset form when 'No' is selected
                  }}
                />
        ...
        <View className="">
          <SubmitButton
            title="Gravar"
            disabled={
              isSubmitting || (!isDirty && isSubmitSuccessful) || !facilityId
            }
