...
export default function LinkEmployeeToWarehouse({
  employerId,
  setValue,
  selectedWarehouse,
}: Props) {
  ...
  const warehouseItems = useMemo(
    () =>
      warehouseList.map((w) => {
        const workplaceTypeLabel = translateWarehouseTypeToPortuguese(
          w.warehouse_type,
        );
        return {
  ...
  return (
    <View>
      ...
      <CustomSelectItemTrigger
        ...
        selectedItem={
          selectedWarehouse?.label || "Seleccione o posto ou armazém ou fábrica"
        }
      />
      <CustomSelectItem
        ...
        emptyMessage={
          isLoading
            ? "Carregando..."
            : "Não há postos ou armazéns sem trabalhador vinculado"
        }
